...
pub fn is_key_released(key_code: KeyCode) -> bool {
    unsafe {
        return sys::is_key_released(key_code as i32);
    }
}
...
pub fn is_key_down(key_code: KeyCode) -> bool {
    unsafe {
        return sys::is_key_down(key_code as i32);
    }
}
...
/// use kioto::keyboard;
///
/// keyboard::is_key_up(keyboard::KeyCode::Space);
/// ```
pub fn is_key_up(key_code: KeyCode) -> bool {
    unsafe {
        return sys::is_key_up(key_code as i32);
    }
}
...
impl Iterator for KeyIter {
    ...
    fn next(&mut self) -> Option<KeyCode> {
        unsafe {
            let raw_code = sys::get_next_key();
            if raw_code != 0 {
                let key_code: KeyCode = mem::transmute(raw_code);
                Some(key_code)
            } else {
                None
            }
